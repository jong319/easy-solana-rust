...
pub struct AssociatedTokenAccount {
    ...
    pub token_program: String // Program owner of the token (e.g Token2022 / Token)
}
...
mod tests {
    ...
    const ASSOCIATED_ACT_WALLET_ADDRESS: &str = "7geCZYWHtghvWj11sb7exvu4uMANfhvGvEvVRRZ8GmSd";
    const ACT_MINT_ADDRESS: &str = "ArDKWeAhQj3LDSo2XcxTUb5j68ZzWg21Awq97fBppump";
    const ASSOCIATED_MIRACOLI_WALLET_ADDRESS: &str = "4rD1Pk3F5R4pUY21vgU3vdnHTGRS97fgD2Y5ozv3FCRx";
    ...
    fn test_get_multiple_associated_token_accounts() {
        ...
        let associated_token_accounts = get_multiple_associated_token_accounts(
                &client,
                vec![ASSOCIATED_ACT_WALLET_ADDRESS, ASSOCIATED_MIRACOLI_WALLET_ADDRESS]
        ...
        let are_owners_the_same = associated_token_accounts.iter().all(|account| account.owner_pubkey.to_string() == WALLET_ADDRESS.to_string());
        assert!(are_owners_the_same);
        let are_tokens_under_token_program = associated_token_accounts.iter().all(|account| account.token_program.to_string() == token_program().to_string());
        assert!(are_tokens_under_token_program);
    }
    ...
    fn failing_test_get_multiple_invalid_associated_token_accounts() {
        ...
        let associated_token_accounts = get_multiple_associated_token_accounts(
                &client,
                vec![WALLET_ADDRESS, ACT_MINT_ADDRESS, MIRACOLI_MINT_ADDRESS]
        ).expect("Failed to get associated token accounts");
    ...
    fn test_derive_associated_token_account_address() {
        ...
        assert!(associated_token_account_address == ASSOCIATED_ACT_WALLET_ADDRESS.to_string())
    }
    ...
    fn test_derive_multiple_associated_token_accounts_address() {
        ...
        wallet_token_mapping.entry(WALLET_ADDRESS.to_string())
        ...
        let associated_token_account_addresses = wallet_associated_account_mapping.get(WALLET_ADDRESS).expect("Wallet does not exist in mapping");
        let is_act_associated_address_found = associated_token_account_addresses.iter().any(|address| address == ASSOCIATED_ACT_WALLET_ADDRESS);
        assert!(is_act_associated_address_found);
        let is_miracoli_associated_address_found = associated_token_account_addresses.iter().any(|address| address == ASSOCIATED_MIRACOLI_WALLET_ADDRESS);
    ...
    fn test_get_all_token_accounts() {
        ...
        let are_tokens_under_same_owner = token_accounts.iter().all(|account| account.owner_pubkey == WALLET_ADDRESS.to_string());
